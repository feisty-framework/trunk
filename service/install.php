...
function get_queries()
{
	...
	$fp = fopen('Feisty-db' . $db_version . '-complete.sql', 'r');
	$queries = stream_get_contents($fp);
	fclose($fp);

    if (!empty($_POST['sample-data'])) {
        $fp = fopen('Feisty-db-sample.sql', 'r');
        $queries .= stream_get_contents($fp);
        fclose($fp);
    }
	...
	// Other queries.
	$query[] = 'REPLACE INTO `' . $data['db_prefix'] . "core_settings` VALUES ('Feisty_crypt_key', '" . $crypt_key . "', '');";

	$query[] = 'REPLACE INTO `' . $data['db_prefix'] . "core_settings` VALUES ('Feisty_from_email', '" . $data['admin_email'] . "', '');";
	$query[] = 'REPLACE INTO `' . $data['db_prefix'] . "core_settings` VALUES ('Feisty_scripts_name_version', '" . $data['application_name'] . "', '');";

	$query[] = 'REPLACE INTO `' . $data['db_prefix'] . "core_settings` VALUES ('Feisty_setting_admin_email', '" . $data['admin_email'] . "', '');";
