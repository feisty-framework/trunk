...
if [ $TYPE -eq 1 -o $TYPE -eq 2 ]; then
	...
	find ./index.php ./includes ./themes/cloud ./language -type f -iname "*.php" | xgettext -o ./language/core.lang.pot --package-name=Feisty --copyright-holder="Jason Schoeman" --msgid-bugs-address=titan@phpdevshell.org --from-code=utf-8 --no-wrap --language=PHP --keyword=_ --keyword=__ --keyword=___ -j -f -
fi

# Extract Plugins Strings
if [ $TYPE -eq 1 -o $TYPE -eq 3 -o $TYPE -eq 4 ]; then
	# Extract Feisty Plugin
	# Set Plugin variable to extract complete directory.
	if [ $TYPE -eq 4 ]; then
		PLUGINLIST=$OPTION
	else
		PLUGINLIST="ControlPanel Feisty FileMan PHPMailer PHPThumbs Pagination Smarty StandardLogin userActions CRUD RedBeanORM BotBlock"
	fi
