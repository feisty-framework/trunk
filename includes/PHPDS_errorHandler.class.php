		...
		foreach ($backtrace as $v) {
			...
			if (isset($v['class'])) {
				...
				if (substr($v['class'], 0, 5) == 'PHPDS') {
					// $call = '<a href="http://doc.phpdevshell.org/Feisty/'.$v['class'].'.html#'.$v['function'].'" target="_blank"><img src="' . $aurl . '/themes/default/images/icons-16/book-question.png" /></a>&nbsp;'.$call;
				}
				$trace .= $call;
			} elseif (isset($v['function'])) {
				$fct = $v['function'];
